...
def main():
    ...
    for docx in docx_files:
        ...
        if matched_item:
            matches.append((docx.name, matched_item.folder_name))
        else:
            unconverted.append(docx.name)

    print(f"Total DOCX: {len(docx_files)}")
    print(f"Total Matches: {len(matches)}")
    print(f"Total Unconverted: {len(unconverted)}")
    ...
    if unconverted:
        ...
        print("\nAll files are matched. Examples of matches:")
        for docx_name, folder_name in matches[:5]:
            print(f"  - {docx_name} -> {folder_name}")
