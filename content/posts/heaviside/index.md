...
**Modern solution:** Taking the Laplace transform \$Y(s)=\mathcal{L}\{y(t)\}\$ with \$sY(s) - y(0)\$ for the derivative, the ODE becomes

``` math
(sY - 0) + 3Y = \frac{6}{s}.
```

So \$Y(s) = \frac{6}{s(s+3)}\$, exactly as above. Partial fractions gives \$Y(s)=2/s - 2/(s+3)\$, and inverse Laplace yields \$y(t) = 2 - 2e^{-3t}\$ for \$t\ge0\$. This agrees with Heaviside’s result.
